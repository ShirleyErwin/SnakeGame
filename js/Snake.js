...
window.onload = function () {
    ...
    var grade = document.querySelector('#grade p');
    var canvas = document.getElementById('GluttonousSnake');
    ...
    function Snake() {
        ...
        //画出4个方块，设置成灰色
        for (var i = 0; i < 4; i++) {
            var rect = new Rect(i * 10, 10, 10, 10, "#fff");
    ...
    Snake.prototype.move = function () {
        var rect = new Rect(this.head.x, this.head.y, this.head.w, this.head.h, "#fff");
    ...
    function getRandomFood() {
        ...
        while (isOnSnake) {
            ...
            isOnSnake = false;
            var indexX = getNumberInRange(0, canvas.width / 10 - 1);
            var indexY = getNumberInRange(0, canvas.height / 10 - 1);
            var rect = new Rect(indexX * 10, indexY * 10, 10, 10, "yellow");
    ...
    function isEat() {
        if (snake.head.x == food.x && snake.head.y == food.y) {
            ...
            setTimeout(function () {
                bgMp.play();
                bgMp.loop=true; //重复背景音乐
                bgMp.currentTime = 3;//音乐开头有空白，所以去掉那段时间的空白
                eatMp.pause();
                eatMp.currentTime = 0;  //因为音乐时长不止1秒，所以播放时间置零
            }, 3000);
            gradeText +=1;
